...
function PhotoInput({inputName,profileImg,setProfileImg}) {
  return (
    <div className="z-10 flex justify-center">
      <label htmlFor="profilePhoto" className="border-2 z-10 relative w-[150px] h-[150px] -mt-20 rounded-full  overflow-hidden  cursor-pointer group">
        ...
        <img src={profileImg ? URL.createObjectURL(profileImg) : "/face.png" /* place holder img */} className="object-cover border-2 border-black rounded-full  z-0" />
      </label>
      <input hidden name={inputName} id="profilePhoto" type="file" accept=".png,.jpeg,.jpg" onChange={async (e) => { setProfileImg(e.target.files[0]) }} />
    </div>
  )
}
